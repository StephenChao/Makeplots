...
class ANALYSIS:
    ...
    def Make_Controlplots_for_0lep(self,selection,selection2,tag,CR=0):
        ...
        if MODE in ["MC","MCvsDATA","DECO"]:
            ...
            self.construct_plot(Nj, "JetMass_Nj2_c_new"    ,selection          ,"",tag  ,50, 0 ,250,"Mj_c, Nj=2,new H4q tagger"                        ,"Events" ,logy,CR);             
            # self.construct_plot(Nj, "JetMass_Nj2_c"    ,selection          ,"",tag  ,50, 0 ,250,"Mj_c, Nj=2, deepWH>1"                        ,"Events" ,logy,CR);             
            # self.construct_plot(Nj, "Pt_ja"    ,selection          ,"",tag  ,50, 100 ,800,"P_T for ja, Nj=2"                        ,"Events" ,logy,CR);             
            # self.construct_plot(Nj, "Pt_Nj2_a"    ,selection          ,"",tag  ,50,0,3500,"P_T for ja, Nj=2, deepWH=1"                        ,"Events" ,logy,CR);             
            # self.construct_plot(Nj, "Pt_Nj2_c"    ,selection          ,"",tag  ,50, 100 ,800,"P_T for jc, Nj=2, deepWH>1"                        ,"Events" ,logy,CR);             
            ...
            # self.construct_plot(Nj,"deepWH_Nj2_a", selection                    ,"",tag  ,50,   0.99,  1.01    ,"deep-WH_a, Nj=2, deepWH=1"           ,"Events" ,logy,CR);
            # self.construct_plot(Nj,"deepWH_Nj2_c", selection                    ,"",tag  ,50,   -0.05,  1.55    ,"deep-WH_c, Nj=2, deepWH>1"           ,"Events" ,logy,CR);
            # self.construct_plot(Nj,"deepH", selection                    ,"",tag  ,40,   0,  1    ,"deep-H_max"           ,"Events" ,logy,CR);
            # self.construct_plot(Nj,"deepWH_Nj2_a", selection ,"",tag,42,-0.05,1.05    ,"deep-WH_a, Nj=2"           ,"Events" ,logy,CR);
            # self.construct_plot(Nj,"deepWH_Nj2_c", selection ,"",tag,42,-0.05,1.05   ,"deep-WH_c, Nj=2"           ,"Events" ,logy,CR);
            # self.construct_plot(Nj,"deepW_Nj2_a", selection                    ,"",tag  ,50,  0.5 , 1.5    ,"deep-W_a, Nj=2, deepWH=1"           ,"Events" ,logy,CR);
            # self.construct_plot(Nj,"deepW_Nj2_c", selection                    ,"",tag  ,50,  -0.05,  1.55   ,"deep-W_c, Nj=2, deepWH>1"           ,"Events" ,logy,CR);
    ...
    def construct_plot(self,Nj,variable,cut,cut1,tag,nbin,min,max,xtitle="",ytitle="",logy=1,CR=0):
        ...
        if options.y=="18"      :path="/eos/user/y/yuzhe/HWW/frame/Custom/Tree/Nj2/";  lumi=59.81;
        ...
        t_Signal1 = TChain("NewTree");  t_Signal1.Add(path+ "Mode.root");  h_Signal1=TH1D("h_Signal1","h_Signal1"+";%s;%s"%(xtitle,ytitle),nbin,min,max);  h_Signal1.Sumw2();
        ...
        t_Signal2 = TChain("NewTree"); t_Signal2.Add(path + "Signal/Nj2/HWW_Nj2_4q_new.root"); h_Signal2 = TH1D("h_Signal2", "h_Signal2" + ";%s;%s" % (xtitle, ytitle), nbin, min, max); h_Signal2.Sumw2();
        ...
        t_QCD     = TChain("NewTree");  t_QCD.Add(    path+"BKG_Nj2/QCD.root"  );  h_QCD    =TH1D("h_QCD"    ,"h_QCD"    +";%s;%s"%(xtitle,ytitle),nbin,min,max);  h_QCD.Sumw2();   
        t_TTbar   = TChain("NewTree");  t_TTbar.Add(  path+"BKG_Nj2/TT.root"   );  h_TTbar  =TH1D("h_TTbar"  ,"h_TTbar"  +";%s;%s"%(xtitle,ytitle),nbin,min,max);  h_TTbar.Sumw2(); 
        t_STop    = TChain("NewTree");  t_STop.Add(   path+"BKG_Nj2/ST.root"   );  h_STop   =TH1D("h_STop"   ,"h_STop"   +";%s;%s"%(xtitle,ytitle),nbin,min,max);  h_STop.Sumw2();  
        t_WJets   = TChain("NewTree");  t_WJets.Add(  path+"BKG_Nj2/WJ.root");  h_WJets  =TH1D("h_WJets"  ,"h_WJets"  +";%s;%s"%(xtitle,ytitle),nbin,min,max);  h_WJets.Sumw2(); 
        t_Rest    = TChain("NewTree");  t_Rest.Add(   path+"BKG_Nj2/Rest.root" );  h_Rest   =TH1D("h_Rest"   ,"h_Rest"   +";%s;%s"%(xtitle,ytitle),nbin,min,max);  h_Rest.Sumw2();   
